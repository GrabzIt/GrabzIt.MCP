  ...
  try {
    if (name === "grabzit_inspect_url") {
      ...
      const rawData = await new Promise<any>((resolve, reject) => {
        client.save_to(null, (error: any, data: any) => {
      ...
      const options: any = {};
      const url = String(args?.url);
      if (args?.delay) options.delay = Number(args?.delay);
...
function buildGrabzitOptions(args: any, format : string) {
  const options: any = {};

  // --- Universal Options (Works across all formats) ---
  if (args.delay) options.delay = Number(args.delay);
  if (args.hideElement) options.hideElement = String(args.hideElement);
  if (args.targetElement) options.targetElement = String(args.targetElement);

  if (format?.toLocaleLowerCase() == 'pdf' || format?.toLocaleLowerCase() == 'docx'){
    if (args.orientation) options.orientation = String(args.orientation);
    if (args.pageSize) options.pageSize = String(args.pageSize);
  } else {
...
async function generateFile(client : any, format : any){
    const fileName = `grabzit_${Date.now()}.${format}`;
    const filePath = path.join(os.tmpdir(), fileName);
    
    await new Promise<void>((resolve, reject) => {
      client.save_to(filePath, (error: any) => {
        if (error) reject(error);
        else resolve();
      });
    });
